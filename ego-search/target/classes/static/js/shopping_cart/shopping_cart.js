    ...
    $('.shipping_address').click(function () {
        ...
        $('.shipping_address').each(function () {
            if ($(this).attr('id')==id) {
                ...
                which = id;
                $("input[name='toPreOrderNum']").val(which);
            }else {
...
//            -号，不能低于 1
    $('span.minus').click(function () {
        ...
        var uid = $(this).parent().siblings(".input_checkbox").children("input").val();
        updateSum(uid,num);
        calSum();
    });
    ...
    function updateSum(uid,sum) {
        $.ajax({
            url: ctx+'/order/updateNum',
            type: 'post',
            dataType: 'JSON',
            data: {uid: uid,num:sum},
        });
    }
